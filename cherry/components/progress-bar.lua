...
function ProgressBar:draw()
  ...
  if (not self.hideText) then
    self:addText()
  end
  if (not self.hideIcon) then
    self:icon()
  end
end
...
function ProgressBar:background()
  if (self.useRects) then
    self.bg =
      display.newRoundedRect(self.display, 0, 0, self.width, self.height, 10)
...
function ProgressBar:progress()
  if (self.useRects) then
    ...
    self.progress:setFillColor(colorize(self.trackColor))
  else
...
function ProgressBar:progressWithAsset()
  ...
  self.progress:setMask(
    graphics.newMask('cherry/assets/images/gui/progress-bar/loading-mask.png')
  )
end
...
function ProgressBar:currentValue()
  if (self.useRects) then
    ...
    return (self.progress.maskX + self:progressWidth() / 1.835) /
...
function ProgressBar:runNextInQueue(force)
  ...
  if (self.currentActionRunning) then
    return
  end
...
function ProgressBar:set(value)
  ...
  if (self.useRects) then
    ...
    if (self.changeBG and value == 100) then
      self:showGreenBG()
    end
  end
end
...
function ProgressBar:reach(step, options)
  ...
  if (value == 100) then
    self.progress:setFillColor(colorize(self.trackFilledColor))
