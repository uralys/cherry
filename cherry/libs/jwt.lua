...
local base64 = require('cherry.libs.base64')

local alg_sign = {
  ['HS256'] = function(data, key)
    return crypto.hmac(crypto.sha256, data, key, true)
  end,
  ['HS384'] = function(data, key)
    return crypto.hmac(crypto.sha384, data, key, true)
  end,
  ['HS512'] = function(data, key)
    return crypto.hmac(crypto.sha512, data, key, true)
  end
}
...
function M.decode(data, key, verify)
  ...
  local ok, header, body, sig =
    pcall(
    function()
      return json.decode(b64_decode(headerb64)), json.decode(
        b64_decode(bodyb64)
      ), b64_decode(sigb64)
    end
  )
