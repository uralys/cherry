...
local function fetchLeaderboard(field, next)
  local url =
    App.API_GATEWAY_URL .. '/leaderboard/' .. App.name .. '/' .. field.name
...
local function displayBoard(field)
  ...
  for i, line in pairs(lines) do
    ...
    timer.performWithDelay(
      math.random(100, 700),
      function()
        if (not board or board.field ~= field) then
          return
        end
        Text:create(
          {
...
local function displayRank(field)
  ...
  local bg =
    display.newRoundedRect(board.rankComponent, 0, 0, BOARD_WIDTH, 100, 10)
...
local function drawButton(num)
  ...
  display.newImage(button, 'cherry/assets/images/gui/buttons/tab-vertical.png')

  ----------------------

  local icon = display.newImage(button, field.image, 0, 10)
