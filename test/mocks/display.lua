...
local function newDisplayObject()
  ...
  _displayObject.setFillColor = function()
    return
  end
  _displayObject.setStrokeColor = function()
    return
  end
  ...
  _displayObject.contentToLocal = function(self, x, y)
    return x - _displayObject.x, y - _displayObject.y
  end
  _displayObject.localToContent = function(self, x, y)
    assert(false, 'not implemented in mock!')
  end
  _displayObject.removeSelf = function()
    _displayObject = {}
  end
  _displayObject.rotate = function(self, rotation)
    _displayObject.rotation = _displayObject.rotation + rotation
  end
  _displayObject.scale = function(self, xScale, yScale)
    _displayObject.xScale = _displayObject.xScale * xScale
    _displayObject.yScale = _displayObject.yScale * yScale
  end
  _displayObject.setMask = function(self, mask)
    _displayObject.mask = mask
  end
  _displayObject.setReferencePoint = function(self, referencePoint)
    _displayObject.referencePoint = referencePoint
  end
  ...
  _displayObject.addEventListener = function(self, name, callback)
    if not _displayObject.eventListeners[name] then
      _displayObject.eventListeners[name] = {}
    end
  ...
  _displayObject.removeEventListener = function(self, name, callback)
    assert(
      _displayObject.eventListeners[name],
      'no such event name: ' .. tostring(name)
    )
  ...
  _displayObject.dispatchEvent = function(self, event)
    ...
    if not listener or #listener == 0 then
      return
    end
  ...
  local metatable = {
    ...
    __index = function(t, key)
      ...
      if key == 'contentWidth' then
        return _displayObject.contentBounds.xMax -
          _displayObject.contentBounds.xMin
      elseif key == 'contentHeight' then
        return _displayObject.contentBounds.yMax -
          _displayObject.contentBounds.yMin
    ...
    __newindex = function(t, key, value)
      if key == 'x' then
        if
          _displayObject.referencePoint == display.TopLeftReferencePoint or
            _displayObject.referencePoint == display.CenterLeftReferencePoint or
            _displayObject.referencePoint == display.BottomLeftReferencePoint
         then
          _displayObject.contentBounds.xMin = value
          _displayObject.contentBounds.xMax = value + _displayObject.width
        elseif
          _displayObject.referencePoint == display.TopRightReferencePoint or
            _displayObject.referencePoint == display.CenterRightReferencePoint or
            _displayObject.referencePoint == display.BottomRightReferencePoint
         then
      ...
      elseif key == 'y' then
        if
          _displayObject.referencePoint == display.TopLeftReferencePoint or
            _displayObject.referencePoint == display.TopCenterReferencePoint or
            _displayObject.referencePoint == display.TopRightReferencePoint
         then
          _displayObject.contentBounds.yMin = value
          _displayObject.contentBounds.yMax = value + _displayObject.height
        elseif
          _displayObject.referencePoint == display.CenterLeftReferencePoint or
            _displayObject.referencePoint == display.CenterReferencePoint or
            _displayObject.referencePoint == display.CenterRightReferencePoint
         then
      ...
      elseif key == 'width' then
        if
          _displayObject.referencePoint == display.TopLeftReferencePoint or
            ...
            _displayObject.referencePoint == display.BottomLeftReferencePoint
         then
          _displayObject.contentBounds.xMax =
            _displayObject.contentBounds.xMin + _displayObject.width
        elseif
          _displayObject.referencePoint == display.TopRightReferencePoint or
            _displayObject.referencePoint == display.CenterRightReferencePoint or
            _displayObject.referencePoint == display.BottomRightReferencePoint
         then
          _displayObject.contentBounds.xMin =
            _displayObject.contentBounds.xMax - _displayObject.width
        else
          _displayObject.contentBounds.xMin =
            _displayObject.contentBounds.x - _displayObject.width * 0.5
          _displayObject.contentBounds.xMax =
            _displayObject.contentBounds.x + _displayObject.width * 0.5
        end
      elseif key == 'height' then
        if
          _displayObject.referencePoint == display.TopLeftReferencePoint or
            ...
            _displayObject.referencePoint == display.TopRightReferencePoint
         then
          _displayObject.contentBounds.yMax =
            _displayObject.contentBounds.yMin + _displayObject.height
        elseif
          _displayObject.referencePoint == display.CenterLeftReferencePoint or
            _displayObject.referencePoint == display.CenterReferencePoint or
            _displayObject.referencePoint == display.CenterRightReferencePoint
         then
          _displayObject.contentBounds.yMin =
            _displayObject.contentBounds.y - _displayObject.width * 0.5
          _displayObject.contentBounds.yMax =
            _displayObject.contentBounds.y + _displayObject.width * 0.5
        else
          _displayObject.contentBounds.xMin =
      ...
      elseif
        key == 'xOrigin' or key == 'yOrigin' or key == 'xReference' or
          key == 'yReference'
       then
        assert(
          false,
          'origin and reference point changes supported only through setReferencePoint'
        )
      end
  ...
  newGroup = function()
    ...
    group.contentWidth = function()
      ...
      if (#group.children == 1) then
        return group.children[1].contentBounds.xMax -
          group.children[1].contentBounds.xMin
      end
      local xMin, xMax
      xMin = #group.children[1].contentBounds.xMin
      xMax = #group.children[1].contentBounds.xMax
      for i = 2, #group.children do
        if xMin > group.children[i].contentBounds.xMin then
          xMin = group.children[i].contentBounds.xMin
        end
        if xMax > group.children[i].contentBounds.xMax then
          xMax = group.children[i].contentBounds.xMax
        end
      end
      return xMax - xMin
    end
    group.contentHeight = function()
      ...
      if (#group.children == 1) then
        return group.children[1].contentBounds.yMax -
          group.children[1].contentBounds.yMin
      end
      local yMin, yMax
      yMin = #group.children[1].contentBounds.yMin
      yMax = #group.children[1].contentBounds.yMax
      for i = 2, #group.children do
        if yMin > group.children[i].contentBounds.yMin then
          yMin = group.children[i].contentBounds.yMin
        end
        if yMax > group.children[i].contentBounds.yMax then
          yMax = group.children[i].contentBounds.yMax
        end
      end
      return yMax - yMin
    end
